...
export async function POST(req, { params }) {
  ...
  if (hasReported(anonId, 'post', id)) {
    return NextResponse.json({ error: 'already_reported' }, { status: 409, headers: carrier.headers });
  }
  const rec = createReport({ anon_id: anonId, target_type: 'post', target_id: id, category, reason });
  if (!rec) {
    return NextResponse.json({ error: 'failed' }, { status: 500, headers: carrier.headers });
  }
