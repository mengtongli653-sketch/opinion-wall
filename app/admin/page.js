...
import { recentPosts, countPosts, countComments, listReports, countPendingReports } from '@/lib/db';
    ...
    <>
      <div className="card">
        ...
        <div className="row" style={{ gap: 0, borderTop: '1px solid var(--border)', marginTop: 6 }}>
          ...
          <div style={{ width: 1, background: 'var(--border)', alignSelf: 'stretch' }} />
          <Stat label={t('admin.stat.pendingReports')} value={stats.pending} />
        </div>
      </div>
      ...
      <div className="card">
        <div style={{ fontWeight: 600, marginBottom: 8 }}>{t('admin.recent.title')}</div>
        ...
        {posts.map((p) => (
          <div key={p.id} className="comment">
            <div className="post-meta" style={{ marginBottom: 4, marginTop: 0 }}>
              ...
              {p.visibility === 'hidden' && <span className="badge" style={{ background: 'var(--danger-soft)', color: 'var(--danger)' }}>🛑 {t('admin.menu.visHidden')}</span>}
              {p.visibility === 'shown' && <span className="badge" style={{ background: 'var(--primary-soft)', color: 'var(--primary)' }}>👁 {t('admin.menu.visShown')}</span>}
            </div>
            <div>
