...
export function createPost({ title, content, author_tag, tag }) {
  const s = load();
  s.meta.post_seq += 1;
  const post = {
    id: s.meta.post_seq,
...
export function recentPosts(limit = 50) {
  const s = load();
  return [...s.posts].sort((a, b) => b.created_at - a.created_at).slice(0, limit);
}
...
export function createComment({ post_id, content, author_tag }) {
  const s = load();
